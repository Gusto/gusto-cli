...
- **Don't link what an outsider can't open.** No SSO- or VPN-gated links: dashboards, log queries, internal docs, ticket trackers. A bare ticket key is the exception, and only in commit subjects, PR titles, PR descriptions, and the "Linked issue" field - the key on its own opens nothing, so it costs an outside reader nothing while keeping work traceable internally. Never as a link, and never in source, comments, tests, or docs, where a reader who can't resolve it just hits a dead reference.
