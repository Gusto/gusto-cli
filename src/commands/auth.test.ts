...
describe("maybeInstallSkillsAfterLogin", () => {
  ...
  test("explicit target dirs install regardless of a persisted 'never' and without prompting", async () => {
    await writeConfig({ skills_auto_install: "never" }, configPaths);
    const dirs = [
...
describe("noToolDetectedWarning", () => {
  test("names every supported tool's home marker and points at the escape hatches", () => {
    ...
    expect(text).toContain("gusto skill install");
  });
});
...
describe("authLoginHandler - --target / GUSTO_SKILLS_TARGET override", () => {
  ...
  test("a valid --target is resolved to dirs and handed to the installer", async () => {
    const { sinks } = captureSinks();
    ...
    const result = await authLoginHandler(
      ...
      {
        login: fakeLogin,
        installSkills: async (_g, _s, deps) => {
          received = deps;
          return [
            { skill: "cash-forecasting", installedAt: "/p", kind: "cursor" as const, action: "installed" as const },
          ];
        },
      },
    )({ ...ctx, sinks });
    expect(result.ok).toBe(true);
    expect(received?.dirs?.map((d) => d.kind)).toEqual(["cursor"]);
  });
});
