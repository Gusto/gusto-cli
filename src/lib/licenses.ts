// License audit + NOTICES generation. Driven by scripts/licenses.ts.
//
//   run("audit")     # fail on any non-allowlisted license
//   run("notices")   # regenerate the NOTICES file
//   run("--check")   # audit + verify NOTICES has no drift
//
...
function audit(): number {
  ...
  console.error(
    "\nReview the license, then either add it to the allowlist in src/lib/licenses.ts\n" +
