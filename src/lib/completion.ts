...
export interface CompletionModel {
  ...
  /** Flag tokens (long and short) for options that require a value (`<value>`), deduped across the
   * tree. The completion walk skips the token following one of these so a space-separated flag
   * value (e.g. `--env sandbox`) is not mistaken for a subcommand. */
  valueFlags: string[];
}
...
export function generateBashCompletion(model: CompletionModel): string {
  ...
  const valueFlagsList = model.valueFlags.join(" ");
