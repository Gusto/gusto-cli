...
6. **Per-employee trajectories.** From the block-1 series, project employer cost per named employee — gross wages + employer taxes + employer benefit contributions — using the same trajectory logic. Note that block 1 does **not** carry per-employee net pay or employee withholding (those appear only as company-level `DebitNetPay` / `DebitTax`), so per-employee figures are employer cost, not take-home.
...
- The per-employee GL block carries gross wages, employer taxes, and employer benefit contributions per person, but **not** net pay or employee withholding per person.
