...
3. **Cash baseline.** `gusto payroll list --processing-status processed --payroll-type regular,off_cycle --include totals --date-filter-by check_date --start-date <window-start> --end-date <~3 months ahead> --sort-order desc` — one call returns both regular and off-cycle runs in the window; partition on each row's `off_cycle` boolean (and read `off_cycle_reason`, e.g. `Bonus`). **Filter by `check_date`, not the default pay-period date** — this is load-bearing: cash leaves the account on the check date, and off-cycle/bonus runs have pay periods that don't line up with the cadence, so the API's default window (pay period, ending _today_) silently drops them and an off-cycle run can be missed entirely (verified: a Bonus run with check date 2026-06-17 is absent under the default filter, present under `check_date`). Set `--end-date` a few months out (the max is ~3 months in the future) to also catch already-processed runs with future check dates. The baseline figure is the **company debit** (`totals.company_debit`); if absent, sum the debit components (`net_pay_debit + tax_debit + reimbursement_debit + child_support_debit` — confirm the keys from the response). Take the most recent regular run's company debit, or an average of the last few, as the **per-period baseline**. Mark processed rows as source `actual`.
   ...
   - **Already-calculated upcoming payrolls (exact near-term rows).** Also list near-term unprocessed payrolls (`gusto payroll list --processing-status unprocessed --include totals`, filtered by `check_date` over the next period or two). For any whose `totals` is already present (someone has calculated the draft), take `totals.company_debit` as an **exact** figure for that check date, mark it source `calculated`, and let it replace the projected row for that period. Leave the rest to projection. Do **not** calculate a draft to obtain this - the skill is read-only (see [Limitations](#limitations)); it only reads totals that already exist.
...
- **An unprocessed payroll is a cash source only once it has been calculated.** `gusto payroll list --processing-status unprocessed --include totals` returns the scheduled paydates but **no `totals`** until a payroll has been _calculated_ (`calculated_at` is null on a freshly created payroll). `gusto payroll calculate <payroll_uuid>` computes those totals, but it is a **write** (it mutates the draft), and this skill is read-only, so **it never calls `calculate` itself** - calculating a draft is the user's or `payroll-prep`'s job. What this skill does do: if an unprocessed payroll in the near-term window has **already** been calculated (`calculated_at` set, `totals` present), read its `totals.company_debit` as the **exact** figure for that check date (source `calculated`) in place of the projected baseline. When `totals` is still null, fall back to the projected baseline as before. So the forecast uses a real calculated figure for a near-term period when one exists, and projects only where it doesn't.
