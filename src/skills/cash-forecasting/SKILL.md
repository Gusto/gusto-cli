...
3. **Cash baseline.** `gusto payroll list --processing-status processed --payroll-type regular,off_cycle --include totals --date-filter-by check_date --start-date <window-start> --end-date <~3 months ahead> --sort-order desc` — one call returns both regular and off-cycle runs in the window; partition on each row's `off_cycle` boolean (and read `off_cycle_reason`, e.g. `Bonus`). **Filter by `check_date`, not the default pay-period date** — this is load-bearing: cash leaves the account on the check date, and off-cycle/bonus runs have pay periods that don't line up with the cadence, so the API's default window (pay period, ending _today_) silently drops them and an off-cycle run can be missed entirely (verified: a Bonus run with check date 2026-06-17 is absent under the default filter, present under `check_date`). Set `--end-date` a few months out (the max is ~3 months in the future) to also catch already-processed runs with future check dates. The baseline figure is the **company debit** (`totals.company_debit`); if absent, sum the debit components (`net_pay_debit + tax_debit + reimbursement_debit + child_support_debit` — confirm the keys from the response). Take the most recent regular run's company debit, or an average of the last few, as the **per-period baseline**. Mark processed rows as source `actual`.
   ...
   - **Already-calculated upcoming payrolls (exact near-term rows).** Enumerate near-term unprocessed payrolls with `gusto payroll list --processing-status unprocessed` (filtered by `check_date` over the next period or two) to get their uuids and `calculated_at`. For any with `calculated_at` set (someone has calculated the draft), read its totals with `gusto payroll show <payroll_uuid> --include totals`. **Use it only if `totals` (and `totals.company_debit`) actually came back non-null** - a calculation can be mid-flight or have failed even with `calculated_at` set, so a null `totals` is not an exact figure. When present, take `totals.company_debit` as an **exact** figure for that check date, mark it source `calculated`, and let it replace the projected row for that period; when `totals` is still null, leave that period to the projected baseline (never record a `calculated` row with a null or zero amount). **Read the totals with `show`, not `list`:** `gusto payroll list --include totals` returns `totals: null` for an unprocessed draft even after it has been calculated (`list` exposes totals only for processed payrolls), so use `list` to find the near-term drafts and their `calculated_at`, then `show` per payroll to get the calculated figure. Leave the rest to projection. Do **not** calculate a draft to obtain this - the skill is read-only (see [Limitations](#limitations)); it only reads totals that already exist.
...
7. **Reconcile + extrapolate.** Per projected period: **Gusto bank debit** = projected `DebitNetPay + DebitTax` (this is `company_debit`); **total employer cost** = projected debit-side accounts (wages + PTO + employer taxes + employer benefit contributions); **benefit-liability outflow** = projected `BenefitLiability` (employer contributions + employee deductions remitted to carriers/401(k) — separate from the Gusto debit). Walk the pay-schedule check dates forward from today to the horizon: for any check date whose unprocessed payroll is already calculated, use its exact `calculated` figure (step 3); mark the rest source `projected`.
8. **Present.**
   - **Cash timeline** — a table `check_date | source (actual|calculated|projected) | company_debit`, sorted by check date, flagging clearly where `projected` rows begin (and any `calculated` near-term rows that came from an already-calculated draft).
...
- **An unprocessed payroll is a cash source only once it has been calculated.** A freshly created draft has no dollar totals (`calculated_at` is null). `gusto payroll calculate <payroll_uuid>` computes those totals, but it is a **write** (it mutates the draft), and this skill is read-only, so **it never calls `calculate` itself** - calculating a draft is the user's or `payroll-prep`'s job. What this skill does do: if a near-term unprocessed payroll has **already** been calculated (`calculated_at` set), read its `totals.company_debit` via `gusto payroll show <payroll_uuid> --include totals` as the **exact** figure for that check date (source `calculated`) in place of the projected baseline. **Read totals with `show`, not `list`:** `gusto payroll list --include totals` returns `totals: null` for an unprocessed draft even after it has been calculated (it exposes totals only for processed payrolls), so `list` only enumerates the near-term drafts and their `calculated_at` while `show` is what returns the calculated totals. When a draft is not yet calculated, fall back to the projected baseline as before. So the forecast uses a real calculated figure for a near-term period when one exists, and projects only where it doesn't.
...
- Running, scheduling, or calculating payroll (this only forecasts and is read-only; it reads `totals` off an already-calculated draft but never calls `gusto payroll calculate`, which mutates the draft, nor any run/schedule command - the V1 CLI doesn't move money).
...
- **When you can't complete the forecast** - a needed command doesn't exist, a required scope isn't granted, or the payroll/ledger data isn't there - offer the user the option to send feedback so the gap reaches Gusto: `gusto feedback --message "<what was missing>" --category <bug|feature_request|general|praise>`. Feedback is an external write, so **sanitize first**: draft a message that describes the gap generically, exclude PII and secrets (employee names, UUIDs, SSN/EIN, wages, hours, addresses, bank/account numbers, tokens) and never paste raw API response or error bodies; then show the user the exact `--message` and `--context` you intend to send and get their explicit approval before sending. It needs no `--confirm` - but that's CLI mechanics (it isn't a gated write), not a waiver of consent; still confirm with the user first. Keep the message concise and under the 5000-character cap - a piped stdin message is silently truncated at that length, so prefer `--message`.
