...
- **`cash_tips` vs `paycheck_tips`** are two distinct built-in earning types. If the owner's input just says "tips" without specifying which, **pause and confirm with the user whether it's `cash_tips` or `paycheck_tips`** before writing - the choice changes net pay. (`cash_tips` = tips the employee already received in cash, reported for taxes; `paycheck_tips` = tips owed to the employee through this paycheck.)
...
- **`skipped_employees` is not the same as "employee not in the file."** A `skipped_employees` entry is the benign case of an employee whose row(s) carry **no input values on any row** - the CLI drops them from the write and reports it. For a multi-job employee split across rows, a blank row for one job does **not** land here as long as another of their rows carries data; that blank row is just padding, so don't expect it back. An employee with **no row at all** isn't skipped; the CLI never sees them, they're left untouched, and they won't appear in `skipped_employees` - the agent surfaces that gap itself in step 5.
...
8. **Calculate the dollar totals and surface the review.** Run `gusto payroll calculate <payroll_uuid> --confirm`. Calculation is a write (it mutates the draft), so in agent mode it needs `--confirm` like the other writes above. It runs asynchronously on the server, but the command waits for the result and returns the payroll with its computed `totals` (gross, net, employer taxes) - you do not poll it yourself (`--timeout` bounds the wait; `--no-wait` opts out and hands back a `calculating` status to read back later). Present the full review: per-employee hours/inputs, the computed dollar totals, plus any `skipped_employees` and the unmatched rows from step 5. (For the per-tax breakdown, additionally read `gusto payroll show <payroll_uuid> --include totals,taxes`.) **If `calculate` returns `insufficient_scope` (a 403)** - the calculate scope may not be granted for this token yet - skip the totals layer and present the hours-only review, telling the user the dollar totals are unavailable until the calculate scope is granted. **If it returns a `calculate_timeout` error** - the server didn't finish computing within the wait budget - the totals aren't ready yet, so present the hours-only review and tell the user to read them back shortly with `gusto payroll show <payroll_uuid> --include totals` (or re-run `calculate` with a longer `--timeout`); don't treat a timeout as calculated. Stop here; the draft stays unprocessed, and the owner reviews and approves the run in the Gusto dashboard.
...
- **`payroll update` writes to a real draft payroll.** Preview with `--dry-run`, confirm the mapping and values with the user (step 5), then re-run with `--confirm` (an agent-mode write is blocked without it).
