...
- `overtime_hours` / `double_overtime_hours` map to Gusto's built-in `Overtime` / `Double overtime` pay types, matched by their fixed seeded names. **Overtime is only valid for non-exempt (FLSA) employees:** writing an `Overtime` / `Double overtime` line - **even a `0`** - for an exempt/salaried employee is rejected with a `422`. Omit the overtime columns entirely for exempt employees rather than sending `0`. Hour types beyond regular/OT/double-OT aren't supported here.
- **The write is all-or-nothing.** If any row is invalid (a negative/non-numeric value, or a pay type not valid for that employee), the API rejects the **entire** file with a `422` and writes nothing - so one exempt-employee overtime line blocks every employee. Read the `422` `details.errors`: it names the offending employee and the reason. This is distinct from `skipped_employees`, the benign case of a row with no input values.
...
- **One invalid row fails the whole write.** The update is all-or-nothing: a negative/non-numeric value, or an `Overtime` line on an exempt/salaried employee, returns a `422` and writes nothing for anyone. Don't send overtime for exempt employees; on a `422`, read `details.errors` to find the offending employee, fix that row, and retry.
